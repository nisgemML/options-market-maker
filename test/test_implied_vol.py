...
def test_iv_round_trip(S, K, T, r, sigma, option_type):
    ...
    try:
        ...
        # Skip ill-conditioned cases where vega ≈ 0 (deep ITM/OTM).
        # When price ≈ intrinsic, many vols produce the same price —
        # the IV problem is not uniquely identified and round-trip error
        # can exceed any fixed tolerance. This is a property of the problem,
        # not the solver.
        vega = float(BlackScholes.vega(S, K, T, r, sigma))
        if vega < 0.01:
            return  # skip near-zero-vega cases
        assert abs(iv - sigma) < 1e-4, f"Round-trip failed: {iv:.6f} != {sigma:.6f}"
